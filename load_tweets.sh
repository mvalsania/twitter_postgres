...
for file in $files; do
    cat "$file" | sed 's/\\u0000//g' | iconv -f utf-8 -t utf-8 -c | psql postgresql://postgres:pass@localhost:21012 -c "COPY tweets_jsonb (data) FROM STDIN csv quote e'\x01' delimiter e'\x02';"
